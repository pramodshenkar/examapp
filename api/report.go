...
func GetStudentReport(studentid string) (models.StudentReport, error) {

	path := fmt.Sprintf("%s%s%s", "database/Report/", studentid, ".json")
	...
	if err := json.Unmarshal([]byte(file), &report); err != nil {
		return models.StudentReport{}, err
	}
